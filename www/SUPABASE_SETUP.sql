-- NovaMed Supabase prototype setup
-- Run this in Supabase SQL Editor.
-- Supabase Auth is not required in this build.
-- Students use a simple name + code inside the app, while Supabase remains active for:
-- 1) global content sync, 2) Storage uploads, 3) full simple student profile sync.

create table if not exists public.novamed_content (
  id text primary key,
  data jsonb not null default '{}'::jsonb,
  updated_at timestamptz not null default now()
);

alter table public.novamed_content enable row level security;
...
-- Prototype policy: lets the static PWA push content using the anon key.
create policy "NovaMed prototype content write"
on public.novamed_content
for all
to anon, authenticated
using (true)
...
insert into public.novamed_content (id, data)
values ('global', '{}'::jsonb)
...
drop trigger if exists set_novamed_content_updated_at on public.novamed_content;
create trigger set_novamed_content_updated_at
before update on public.novamed_content
for each row execute function public.set_updated_at();

-- Student profile table + migration + RLS policies.
...
-- Storage bucket. This build expects one bucket with folders/prefixes:
-- bucket: novamed
-- video folder: focus video
-- file folder: novamed-files
insert into storage.buckets (id, name, public)
values ('novamed', 'novamed', true)
...
drop policy if exists "NovaMed public video read" on storage.objects;
drop policy if exists "NovaMed public file read" on storage.objects;
drop policy if exists "NovaMed prototype video upload" on storage.objects;
drop policy if exists "NovaMed prototype file upload" on storage.objects;
...
-- Prototype upload policy: lets the static frontend upload videos/files with the anon key.
-- Replace with a protected admin-only route before a real public launch.
create policy "NovaMed prototype storage upload"
on storage.objects
for all
to anon, authenticated
using (bucket_id = 'novamed')
with check (bucket_id = 'novamed');

-- Ask PostgREST/Supabase API to refresh the schema cache after creating student_profiles.
notify pgrst, 'reload schema';
